...
function find(db_path,db_name,db_colle,condition,callback){
	...
	if(util.isArray(condition)){
		if(condition.length == 1){
			//if condition length is one,can't use loop,take easy!
			Stream.readFile(relpath,function(data){
				var translate = JSON.parse(data).spkeys;
				for(var i = 0;i < translate.length;i++){
					var keys = Object.keys(condition[0])[0];
					if(translate[i][ keys ] == condition[0][keys]){
						callback(translate[i]);
						break;
					}
					if(i==translate.length-1 && translate[i][ keys ] != condition[0][keys]){
